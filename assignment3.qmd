...
ordinal_regression_model_age <- polr(Tondu_factor ~ age_factor, data = TEDS_2016, Hess=TRUE)
pred_data_age <- data.frame(age_factor = levels(TEDS_2016$age_factor))
pred_data_age_probs <- predict(ordinal_regression_model_age, newdata = pred_data_age, type = "probs")
long_data <- pivot_longer(as.data.frame(pred_data_age_probs), cols = -age_factor, names_to = "Tondu_Response", values_to = "Probability")
ggplot(long_data, aes(x = age_factor, y = Probability, color = Tondu_Response, group = Tondu_Response)) + geom_line() + geom_point() + theme_minimal() + labs(x = "Age Group", y = "Predicted Probability", title = "Predicted Probability of Tondu Responses by Age Group")
